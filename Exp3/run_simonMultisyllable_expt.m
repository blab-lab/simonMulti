function expt = run_simonMultisyllable_expt(expt, bTestMode)
...
% one word receives no shift. There is also a distractor word which also
% receives no shift.
%
...
% correctExptText = sprintf(['\nWait! SimonMultisyllable has multiple versions, and this is NOT the most recent version. ' ...
%     '\nThis is the first version (simonMultisyllable). ' ...
%     '\nAre you sure that you want to run the FIRST VERSION of simonMultisyllable?']);
% bCorrectExpt = askNChoiceQuestion(correctExptText, {'yes' 'no'});
% if strcmp(bCorrectExpt, 'no')
%     return;
% end

%% Experiment Setup
expt.name = 'simonMultisyllable';
...
expt.words = {'pedigree', 'pedicure', 'pedestal', 'carbonate'}; % pedestal always = noShift
...
if bRunLPCcheck
    ...
    input('Read instructions for OST Check pretest phase then press ENTER to continue.', 's');
end
...
while ~bGoodOSTs
    ...
    % run ost_check phase
    exptost = run_simonMultisyllable_audapter(exptost); 
...
input('Read instructions for Main Phase then press ENTER to continue.', 's');
...
if exist(permsPath,'dir')
    ...
    % (for simonMultisyllable in particular, there's 2 permutations)
    expt.permIx = randi(2);
...
expt.allShifts(expt.allWords == 3) = 1; % noShift for pedestal
expt.allShifts(expt.allWords == 4) = 1; % noShift for carbonate

expt.listShifts(expt.allWords == 1) = expt.shifts{expt.permList{1}};
expt.listShifts(expt.allWords == 2) = expt.shifts{expt.permList{2}};
expt.listShifts(expt.allWords == 3) = 0; %noShift for pedestal
expt.listShifts(expt.allWords == 4) = 0; %noShift for carbonate
...
%% run main experiment
expt = run_simonMultisyllable_audapter(expt);
