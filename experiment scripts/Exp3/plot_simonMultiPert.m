function plot_simonMultiPert(exptName)

% Plot data for simon experiments with mutliple words. Based on plotting for simonSinglWord.
%
% 2022-02 BP init.
...
filenameTableData = sprintf('statTable_%ds',nSubs);
load(fullfile(get_acoustLoadPath(exptName),strcat(filenamePlotData,'.mat')));
load(fullfile(get_acoustLoadPath(exptName),strcat(filenameTableData,'.mat')));

load(fullfile(dataPaths{1},'expt'));
words = expt.words;
nWords = length(words);
phases = fieldnames(ix);
nPhases = length(phases)-1; %don't draw a line after the first block of the washout phase

nBins = size(fdata_norm.f1{1},2);
plotColors = [  .9 0 0;
                0 0 .9;
                0 0 0;
                0.5 0.5 0.5];
...
for w = 1:nWords
    errorbar(1:nBins,mean(fdata_norm_flip.f1{w}),ste(fdata_norm_flip.f1{w}),'o-','Color',plotColors(w,:),'MarkerFaceColor',plotColors(w,:));
end
hline(0,'k',':');
ylim([-30 30])
ylabel('F1 (mels)');
set(gca,'XTick', [ix.baseline/2 + .5, ix.baseline + (ix.ramp-ix.baseline)/2 + .5, ix.ramp + (ix.adapt-ix.ramp)/2+.5, ix.adapt + (nBins - ix.adapt)/2  + .5],...
    'XTickLabel', phases,'YTick',[-30 0 30])
% legend(words);
for p = 1:nPhases
...
h2(1) = figure; hold on;
for w = 1:nWords
    errorbar(1:nBins,mean(fdata_norm_flip.f1{w}),ste(fdata_norm_flip.f1{w}),'o-','Color',plotColors(w,:),'MarkerFaceColor',plotColors(w,:),'LineWidth',lineWidth);
end
ylim([-30 30])
hline(0,'k',':');
for p = 1:nPhases
...
plot([0:ix.baseline ix.baseline+0.5 ix.baseline+1:ix.ramp ix.ramp+0.5 ix.ramp+1:ix.adapt ix.adapt+0.5 ix.adapt+0.5 ix.adapt+1:nBins+1]...
    ,[zeros(1,ix.baseline+1) rampVals([1 2 4 6 7]) repmat(-125, 1, ix.adapt-ix.ramp+1) zeros(1,nBins - ix.adapt + 2)],'-','Color',plotColors(2,:),'LineWidth',lineWidth);
plot([0:ix.baseline ix.baseline+0.5 ix.baseline+1:ix.ramp ix.ramp+0.5 ix.ramp+1:ix.adapt ix.adapt+0.5 ix.adapt+0.5 ix.adapt+1:nBins+1]...
    ,[zeros(1,ix.baseline+1) -1*rampVals([1 2 4 6 7]) repmat(125, 1, ix.adapt-ix.ramp+1) zeros(1,nBins - ix.adapt + 2)],'-','Color',plotColors(1,:),'LineWidth',lineWidth);
plot(1:nBins,zeros(1,nBins),'-','Color','k','LineWidth',lineWidth);
ylim([-125 125])
hline(0,'k',':');
for p = 1:nPhases
    vline(ix.(phases{p})+0.5,'k','--');
end
set(gca, 'YTick', [-125 0 125],'XTick',[])
makeFig4Screen

h_all(2) = figure('Units','centimeters','Position',[figpos(1:3) figpos(4)]); hold on;
