function [expt] = run_simonMultisyllable_v2_audapter(expt)
% Companion function of run_simonMultisyllable_v2_expt. Shows words on screen,
...
helpersDir = fullfile(get_gitPath('current-studies'), expt.name);
...
p.bRatioShift = 0;
p.bMelShift = 1;
p.rmsForgFact = 0.89; % lower forgetting factor (ie, forgets faster)
...
for itrial = firstTrial:lastTrial
    ...
    while ~bGoodTrial
        ...
        for i = 0:8
            set_pcf(expt.trackingFileLoc, expt.trackingFileName, 'space', i, 'fmtPertPhi', expt.shiftAngles(itrial)); %currently set up with same angle across all experiments but here for potential future use
        end
        for i = 0:3 %only change magnitude for V1. magnitude for other vowels is always 0
            set_pcf(expt.trackingFileLoc, expt.trackingFileName, 'space', i, 'fmtPertAmp', expt.shiftMags(itrial));
        end
