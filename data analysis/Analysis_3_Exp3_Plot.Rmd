...
ggplot(data = temp2, aes(x = Group1, y = F1_Change, color = Direction)) +
  ...
  ggtitle("Expriment 3_Group1")

ggsave(filename = "Exp3_Group1.png",
       dpi = 600,
       units = "in",
       width = 7.5,
       height = 3)
...
```{r}
temp2 <- data.frame(rep(c(1:var1), each = length(unique(data3$Word)) * 2, length.out = max(data3$Trial)))

temp1 <- cbind(temp1, temp2)

colnames(temp1) <- c("Trial", "Group1", "Group2")

data3$Group2 <- lookup(terms = data3$Trial,
                       key.match = temp1$Trial,
...
ggplot(data = temp2, aes(x = Group2, y = F1_Change, color = Direction)) +
  ...
  #scale_y_continuous(expand = c(0, 0), limits = c(-55, 55)) +
  scale_color_manual(values = c("red", "blue", "gray")) +
  ggtitle("Expriment 3_Group2")

ggsave(filename = "Exp3_Group2.png",
       dpi = 600,
       units = "in",
       width = 7.5,
       height = 3)
